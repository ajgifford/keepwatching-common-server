 ...
 */
export async function getContentDiscoveryStats(profileId: number, days: number = 30): Promise<ContentDiscoveryStats> {
  return await DbMonitor.getInstance().executeWithTiming('getContentDiscoveryStats', async () => {
    const connection = await getDbPool().getConnection();
    try {
      // Get last content added and content added in the selected period
      const [additionRows] = await connection.execute<ContentAdditionDataRow[]>(
        ...
        SELECT
          ...
          (SELECT COUNT(*) FROM show_watch_status WHERE profile_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as shows_added_30_days,
          (SELECT COUNT(*) FROM movie_watch_status WHERE profile_id = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)) as movies_added_30_days
        `,
        [profileId, profileId, profileId, days, profileId, days],
      );
      ...
      // Get watch completion data for the selected period

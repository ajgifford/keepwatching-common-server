    ...
    try {
      ...
      const [rows] = await connection.query<AccountRankingRow[]>(
        ...
        LEFT JOIN (
          ...
          JOIN profiles p ON p.id = ews.profile_id
          WHERE ews.status = 'WATCHED'
          GROUP BY p.account_id
        ) as episode_stats ON episode_stats.account_id = a.id
        LEFT JOIN (
          SELECT 
            p.account_id,
          ...
          JOIN profiles p ON p.id = mws.profile_id
          WHERE mws.status = 'WATCHED'
          GROUP BY p.account_id
        ) as movie_stats ON movie_stats.account_id = a.id
        LEFT JOIN (
          SELECT 
            p.account_id,
            MAX(GREATEST(
          ...
          LEFT JOIN movie_watch_status mws ON mws.profile_id = p.id AND mws.status = 'WATCHED'
          GROUP BY p.account_id
    ...
    try {
      const [rows] = await connection.query<AccountHealthRow[]>(
        ...
        LEFT JOIN (
          ...
          WHERE ews.status = 'WATCHED'
          GROUP BY p.account_id
        ) as all_episodes ON all_episodes.account_id = a.id
        LEFT JOIN (
          SELECT 
            p.account_id,
            COUNT(*) as episodes_watched
          ...
          GROUP BY p.account_id
        ) as recent_episodes ON recent_episodes.account_id = a.id
        LEFT JOIN (
          SELECT 
            p.account_id,
            MAX(GREATEST(
        ...
        GROUP BY a.id, a.email, a.email_verified, a.created_at, 

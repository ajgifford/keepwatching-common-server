import {
  ...
  ContentUpdates,
  ContentUpdatesRow,
  transformContentReferenceRow,
  transformContentUpdates,
} from '../../types/contentTypes';
import {
  MovieReferenceRow,
  ProfileMovieDetailsRow,
  ProfileMovieRow,
...
export async function saveMovie(createRequest: CreateMovieRequest): Promise<number> {
  ...
  try {
    ...
    return await DbMonitor.getInstance().executeWithTiming(
      ...
      async () => {
        return await transactionHelper.executeInTransaction(async (connection) => {
          ...
          if (createRequest.genre_ids && createRequest.genre_ids.length > 0) {
            const genrePromises = createRequest.genre_ids.map((genreId) =>
              saveMovieGenre(insertId, genreId, connection),
            );
          ...
          if (createRequest.streaming_service_ids && createRequest.streaming_service_ids.length > 0) {
            const servicePromises = createRequest.streaming_service_ids.map((serviceId) =>
              saveMovieStreamingService(insertId, serviceId, connection),
            );
      ...
      1000,
      { content: { id: insertId, type: 'movie' } },
    );
  } catch (error) {
...
export async function updateMovie(updateRequest: UpdateMovieRequest): Promise<boolean> {
  ...
  try {
    return await DbMonitor.getInstance().executeWithTiming(
      ...
      1000,
      { content: { id: updateRequest.id, type: 'movie' } },
    );
  } catch (error) {
...
/**
 * Gets a specific movie with extended details for a profile with watch status information
 *
 * This function retrieves a single movie from a profile's watchlist
 * along with its watch status.
 *
